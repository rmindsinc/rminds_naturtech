...
class OverdueBillStart(models.TransientModel):
    _name = "overdue.bill.start"
    _description = "Wizard to reminder overdue vendor's Bill"
    ...
    payment_ids = fields.Many2many(
        "overdue.bill.start.payment", readonly=True
    )
    ...
    vendor_policy = fields.Selection(
        selection=[
            ('last_reminder','Last Reminder'),
            ('last_bill','Last Bill'),
            ('bill_contact','Bill Contact')
        ],required=True, string="Contact to Remind"
    )
        ...
        res.update(
            {
                ...
                "vendor_policy": company.overdue_reminder_partner_policy,
            }
        )
        return res
    ...
    def bill_run(self):
        ...
        overdue_bill_step= self.env["overdue.bill.step"]
        user_id = self.env.user.id
        existing_actions = overdue_bill_step.search([("user_id", "=", user_id)])
        existing_actions.unlink()
        payment_journals = account_journal.search(
            [
                ("company_id", "=", self.company_id.id),
                ("type", "in", ("bank", "cash")),
            ]
        )
        purchase_journals = account_journal.search(
            [
                ("company_id", "=", self.company_id.id),
                ("type", "=", "purchase"),
            ]
        ...
        rg_res = account_move.read_group(
            domain,
        ...
        for rg_re in rg_res_sorted:
            ...
            if vals:
                action = overdue_bill_step.create(vals)
                action_ids.append(action.id)
        if not action_ids:
            raise UserError(_("There are no overdue bill's."))
        if self.interface == "onebyone":
            xid = MOD + ".overdue_bill_step_onebyone_action"
            action = self.env.ref(xid).sudo().read()[0]
            action["res_id"] = action_ids[0]
        elif self.interface == "mass":
            action = overdue_bill_step.goto_list_view()
        return action
    ...
    def _prepare_reminder_step(
        ...
        if any(
            ...
            logger.info(
                "Skipping vendor %s that has at least one bill "
        ...
        unrec_payments = account_move_line.search(
            unrec_domain
            + [
                ("journal_id", "in", payment_journals.ids),
            ]
        )
        unrec_refunds = account_move_line.search(
            unrec_domain
            + [
        ...
        if self.vendor_policy == "last_reminder":
        ...
        elif self.vendor_policy == "last_bill":
        ...
        elif self.vendor_policy == "bill_contact":
            partner_id = commercial_partner.address_get(["invoice"])["invoice"]
    ...
    def autopost_overdue_bill(self):
        start_rec_default_values = self.env['overdue.bill.start'].default_get([])
        start_rec_default_values['up_to_date'] = True
        start_rec = self.env['overdue.bill.start'].create(start_rec_default_values)
        action = start_rec.bill_run()
        res_id = action.get('res_id',False)
        while res_id:
            step_rec = self.env['overdue.bill.step'].browse(action.get('res_id'))
            step_rec.validate()
            action = step_rec.next_step()
            res_id = action.get('res_id',False)

    def _autopost_overdue_yearly(self):
        print("\n\n\n\n working")

class OverdueBillStartPayment(models.TransientModel):
    _name = "overdue.bill.start.payment"
    _description = "Status of payments"

    wizard_id = fields.Many2one("overdue.bill.start", ondelete="cascade")
...
class OverdueBillStep(models.TransientModel):
    _name = "overdue.bill.step"
    _description = "Overdue bill wizard step"

    partner_id = fields.Many2one(
        "res.partner", required=True, string="Billing Contact"
    )
    ...
    commercial_partner_id = fields.Many2one(
        "res.partner", string="Vendor", readonly=True, required=True
    )
    ...
    bill_ids = fields.Many2many(
        "account.move", string="Overdue Bill", readonly=True
    )
    company_id = fields.Many2one(
        "res.company",
        readonly=True,
    ...
    unreconciled_move_line_normal = fields.Boolean(
        ...
        "reason not to be reconciled with an open bill"
    )
    ...
    def next_step(self):
        ...
        if left:
            action = (
                self.env.ref(MOD + ".overdue_bill_step_onebyone_action")
                .sudo()
                .read()[0]
            )
            action["res_id"] = left.id
        else:
            action = self.env.ref(MOD + ".overdue_bill_reminder_end_action").sudo().read()[0]
        return action

    def goto_list_view(self):
        action = (
            self.env.ref(MOD + ".overdue_bill_step_mass_action").sudo().read()[0]
        )
        return action
    ...
    def _prepare_mail_activity(self):
        ...
        if not self.activity_user_id:
            raise UserError(
                _(
                    "For the reminder of vendor '%s', you must assign someone "
                    "for the activity."
                )
        ...
        if not self.activity_deadline:
            raise UserError(
                _(
                    "For the reminder of vendor '%s', the deadline is missing "
                    "for the activity."
                )
                % self.commercial_partner_id.display_name
            )
    ...
    def check_warnings(self):
        ...
        for rec in self:
            ...
            if (
                ...
                raise UserError(
                    _(
                        "Vendor '%s' has unreconciled payments/refunds. "
    ...
    def validate(self):
        overdue_reminder_action = self.env["overdue.reminder.action"]
        ...
        for rec in self:
            ...
            # print("\n\n\n vals: ",vals)
            overdue_reminder_action.create(vals)
            if rec.create_activity:
                mail_activity.create(self._prepare_mail_activity())
    ...
    def print_letter(self):
        ...
        action = action = (
            self.env.ref(MOD + ".overdue_bill_step_report")
    ...
    def print_invoices(self):
        action = (
            ...
            .with_context({"discard_logo_check": True})
            .report_action(self.bill_ids.ids)
        )
        return action
    ...
    def total_residual(self):
        ...
        for inv in self.bill_ids:
            res[inv.currency_id] += inv.amount_residual * (
                inv.move_type == "in_refund" and -1 or 1
            )
        return res.items()
...
class OverdueBillReminderEnd(models.TransientModel):
    _name = "overdue.bill.reminder.end"
    _description = "Congratulation end screen for overdue bill reminder wizard"

class OverdueBillRemindMassUpdate(models.TransientModel):
    _name = "overdue.bill.reminder.mass.update"
    ...
    def bill_run(self):
        self.ensure_one()
        assert self._context.get("active_model") == "overdue.bill.step"
        actions = self.env["overdue.bill.step"].browse(
