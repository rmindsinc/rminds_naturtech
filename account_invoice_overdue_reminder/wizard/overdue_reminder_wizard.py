...
class OverdueReminderStart(models.TransientModel):
    ...
    def run(self):
        ...
        invs = amo.search(domain)
        if self.env.context.get('reminder_cron'):
            date_today = date.today()
            reminder = self.env['res.config.settings'].default_get([])
            cron_counter = reminder['overdue_limit_counter']
            start_rec_default_values = self.env['overdue.reminder.start'].default_get([])
            cron_days = start_rec_default_values['min_interval_days']
            invs = invs.filtered(lambda inv:(inv.overdue_reminder_counter== 0) or (inv.overdue_reminder_counter<cron_counter and (inv.last_reminder_mail.date() +timedelta(days = cron_days)) == date_today))
        rg_res = amo.read_group(
            [('id','in',[inv.id for inv in invs])],
    ...
    def autopost_overdue_invoice(self):
        start_rec_default_values = self.env['overdue.reminder.start'].default_get([]) 
        start_rec_default_values['up_to_date'] = True
        start_rec = self.env['overdue.reminder.start'].create(start_rec_default_values)
        context = self.env.context
        context.update({'reminder_cron':1})
        action = start_rec.with_context(context).run()
        res_id = action.get('res_id',False)
        while res_id:
            step_rec = self.env['overdue.reminder.step'].browse(action.get('res_id',))
            step_rec.validate()
            for d in step_rec.invoice_ids:
                d.last_reminder_mail = datetime.datetime.now()
...
class OverdueReminderStep(models.TransientModel):
    ...
    last_reminder_mail = fields.Datetime(
        readonly=True,
        help="Last Email send to customer reminder Date",
    )
