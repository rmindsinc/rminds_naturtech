...
class AccountInvoiceOverdueReminder(models.Model):
    ...
    bill_id = fields.Many2one(
        "account.move", string="Bill", ondelete="cascade", readonly=True
    )
    ...
    @api.constrains("bill_id")
    def bill_id_check(self):
        for action in self:
            if action.bill_id and action.bill_id.move_type != "in_invoice":
                raise ValidationError(
                    _(
                        "An overdue reminder can only be attached "
                        "to a vendor invoice"
    ...
    @api.depends("bill_id", "counter")
    def name_get(self):
        res = []
        for rec in self:
            name = _("%s Reminder %d") % (rec.bill_id.name, rec.counter)
            res.append((rec.id, name))
        return res
