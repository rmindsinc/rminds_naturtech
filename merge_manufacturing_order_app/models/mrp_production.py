...
class MrpProduction(models.Model):
    ...
    def _compute_mrp_production_child_count(self):
        for production in self:
            production.mrp_production_child_count = self.env['mrp.production'].search_count([('origin', 'ilike', production.name)])
    ...
    def _compute_mrp_production_source_count(self):
        for production in self:
            print(production.origin.split(", "),"production")
            me = self.env['mrp.production'].search([('name', 'in', production.origin.split(", "))])
            print(me,"meeeeeeeeeeeeee")
            production.mrp_production_source_count = self.env['mrp.production'].search_count([('name', 'in', production.origin.split(", "))])


    def action_view_mrp_production_sources(self):
        self.ensure_one()
        mrp_production_ids = self.env['mrp.production'].search([('name', 'in', self.origin.split(", "))])
        action = {
            'res_model': 'mrp.production',
            'type': 'ir.actions.act_window',
        }
